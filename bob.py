...
DISAPPEAR_TIMEOUT = 0.35       # Задержка проверки: ловля продолжается, если шкала вернется за 0.35 сек
...
def input_worker():
    while True:
        ...
        if command == "click":
            ...
            # Гарантированно отжимаем ЛКМ перед сбором
            pydirectinput.mouseUp()
            
            print("[ПОТОК ВВОДА] Шкала отсутствовала 0.35 сек. Начинаем 2 секунды ожидания поимки...")
            time.sleep(2.0)
            ...
            safe_click()
            time.sleep(1.5)  # Задержка, чтобы персонаж успел сделать взмах
            print("[ПОТОК ВВОДА] Ожидание новой рыбы...")
...
def analyze_static(bgr):
    ...
    # Если на экране нет ни зоны, ни маркера — мини-игра не активна
...
def draw_analysis(bgr, res, fps):
    ...
    text = "ACTIVE" if AUTOCLICKER_ENABLED else "PAUSED"
    cv2.putText(out, text, (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
    cv2.putText(out, f"{fps:.0f} FPS", (10, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
...
def run_live():
    ...
    with mss.mss() as sct:
        print("Бот готов к бесконечному циклу. Нажмите K для старта.")
        ...
        while True:
            ...
            if AUTOCLICKER_ENABLED:
                if res is not None and ("zone_bottom" in res or "marker_center_y" in res):
                    # Шкала на экране — идет мини-игра
                    track_was_present = True
                    is_collecting = False
                    last_seen_time = now  # Обновляем таймер: шкалу только что видели
                    ...
                    # Шкалы нет на этом кадре
                    if is_pressing:
                        input_queue.put("up")
                        is_pressing = False
                    
                    # Запускаем сбор ТОЛЬКО если шкала отсутствует дольше 0.35 секунды
                    if track_was_present and not is_collecting:
                        if (now - last_seen_time) >= DISAPPEAR_TIMEOUT:
                            print(f"[БОТ] Шкала пропала более чем на {DISAPPEAR_TIMEOUT}s. Подтверждение поимки...")
                            is_collecting = True
                            track_was_present = False
